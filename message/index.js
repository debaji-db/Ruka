...
const { nsfw, weeaboo, downloader, fun, misc, toxic, spam} = require('../lib')
     ...
     case prefix+ 'covid':
        ...
        break
            ...
            case prefix+ 'ttp': // CHIKAA CHANTEKKXXZZ
                ...
                misc.ttp(q)
                    .then(async (res) => {
                        await ruka.sendImageAsSticker(from, res.base64, { author: authorWm, pack: packWm })
            ...
            case prefix+ 'tafsir':
                if (!isRegistered) return await ruka.reply(from, eng.notRegistered(), id)
                if (args.length === 0) return ruka.reply(from, `Untuk menampilkan ayat Al-Qur'an tertentu beserta tafsir dan terjemahannya\ngunakan ${prefix}tafsir surah ayat\n\nContoh: ${prefix}tafsir Al-Mulk 10`, id)
                await ruka.reply(from, eng.wait(), id)
                ...
                if (!isNaN(nomerSurah)) {
                    ...
                    await ruka.reply(from, pesan, id)
                }
            break
            ...
            break
            case prefix+ 'play':  
            if (!q) return await ruka.reply(from, 'Wrong Format', id)
            await ruka.reply(from, 'WAIT' ,id)
            // const vserplay2 = body.slice(7)
            const getsong = await axios.get(`https://api.zeks.xyz/api/ytplaymp3/2?apikey=apivinz&q=${q}`)
            if (getsong.data.status === false) {
                await ruka.reply(from, getsong.data.message, id)
            } else if (Number(getsong.data.result.size.split('MB')[0]) >= 50.00) {
                await ruka.reply(from, 'music must be under 50 MB!', id)
            } else {
                await ruka.sendFileFromUrl(from, getsong.data.result.thumb, 'thumb.jpg', `Now playing...\n\n📝 Title: ${getsong.data.result.title}\n\n───────────⚪───────────\n♪♫•♪ ♪♫•♪ ♪♫•♪\n───────────⚪───────────\n💽 *Size: ${getsong.data.result.size}\n📻 Type: Mp3\n🔗 Link Download: ${getsong.data.result.link}\n_Song is coming... Wait_`, id)
                await ruka.sendFileFromUrl(from, getsong.data.result.link, 'play.mp3' , '', id)
           ...
           // const vserplay2 = body.slice(7)
            ...
            case prefix+ 'chara':
                ...
                return ruka.sendFileFromUrl(from, dt.url, 'file.jpg', dt.msg, id)
            break
            case prefix+ 'images':
                if (!isRegistered) return await ruka.reply(from, eng.notRegistered(), id)
...
await ruka.sendFile(from , './media/menu.jpg', 'menu.jpg',sentence)
} else if (per < 50) {
...
break
            ...
            case prefix+ 'sgif':
                ...
                if (isMedia && isVideo || isGif || isQuotedVideo || isQuotedGif) {
                    ...
                    await ruka.sendMp4AsSticker(from, videoBase64, null, { stickerMetadata: true, author: authorWm, pack: packWm, keepScale: true, fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
                            .then(() => {
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                            })
                    } catch (err) {
